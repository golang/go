...
func callTrampoline(fnAddr uintptr) []byte {
	...
	copy(code[off:], mU32(0x27BDFFF8))
	off += 4 // addiu $sp, $sp, -8
	copy(code[off:], mU32(0xAFBF0004))
	off += 4 // sw $ra, 4($sp)
	copy(code[off:], mU32(0xAFBE0000))
	off += 4 // sw $fp, 0($sp)
	copy(code[off:], mU32(0x03A0F025))
	off += 4 // move $fp, $sp

	copy(code[off:], mU32(0x3C190000|uint32(hi)))
	off += 4 // lui $t9, hi
	copy(code[off:], mU32(0x37390000|uint32(lo)))
	off += 4 // ori $t9, $t9, lo
	...
	copy(code[off:], mU32(0x8FBE0000))
	off += 4 // lw $fp, 0($sp)
	copy(code[off:], mU32(0x8FBF0004))
	off += 4 // lw $ra, 4($sp)
	copy(code[off:], mU32(0x27BD0008))
	off += 4 // addiu $sp, $sp, 8
...
func callTrampolineStackMaps() []jit.StackMap {
	return []jit.StackMap{{PCOffset: 32, HasUnwind: true, CallerPCOffset: 4, CallerSPOffset: 8}}
}
