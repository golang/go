...
func callTrampoline(fnAddr uintptr) []byte {
	...
	copy(code[off:], mU32(0x67BDFFF0))
	off += 4 // daddiu $sp, $sp, -16
	copy(code[off:], mU32(0xFFBF0008))
	off += 4 // sd $ra, 8($sp)
	copy(code[off:], mU32(0xFFBE0000))
	off += 4 // sd $fp, 0($sp)
	copy(code[off:], mU32(0x03A0F025))
	off += 4 // move $fp, $sp (or $fp, $sp, $zero)

	// Load address into $t9
	copy(code[off:], mU32(0x3C190000|uint32(w0)))
	off += 4 // lui $t9, w0
	copy(code[off:], mU32(0x37390000|uint32(w1)))
	off += 4 // ori $t9, $t9, w1
	copy(code[off:], mU32(0x0019CC38))
	off += 4 // dsll $t9, $t9, 16
	copy(code[off:], mU32(0x37390000|uint32(w2)))
	off += 4 // ori $t9, $t9, w2
	copy(code[off:], mU32(0x0019CC38))
	off += 4 // dsll $t9, $t9, 16
	copy(code[off:], mU32(0x37390000|uint32(w3)))
	off += 4 // ori $t9, $t9, w3

	copy(code[off:], mU32(0x0320F809))
	off += 4 // jalr $t9
	copy(code[off:], mU32(0x00000000))
	off += 4 // nop (delay slot)

	copy(code[off:], mU32(0xDFBE0000))
	off += 4 // ld $fp, 0($sp)
	copy(code[off:], mU32(0xDFBF0008))
	off += 4 // ld $ra, 8($sp)
	copy(code[off:], mU32(0x67BD0010))
	off += 4 // daddiu $sp, $sp, 16
	copy(code[off:], mU32(0x03E00008))
	off += 4 // jr $ra
	copy(code[off:], mU32(0x00000000))
	off += 4 // nop
	return code[:off]
...
func callTrampolineStackMaps() []jit.StackMap {
	return []jit.StackMap{{PCOffset: 48, HasUnwind: true, CallerPCOffset: 8, CallerSPOffset: 16}}
}
