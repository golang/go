...
func addCovMeta(p unsafe.Pointer, dlen uint32, hash [16]byte, pkpath string, pkid int, cmode uint8, cgran uint8) uint32 {
	...
	if pkid != -1 {
		...
		if _, ok := covMeta.pkgMap[pkid]; ok {
			throw("runtime/coverage.addCovMeta: coverage package map collision")
		}
...
// for the currently executing instrumented program.
...
// more on why hard-coded package IDs are needed).
	...
	for _, c := range s.counterlist {
		...
		for i := 0; i < len(sd); i++ {
			...
			if ipk == 0 {
				...
				// runtime/coverage.addCovMeta), so subtract off 1 here to form
				// the real package ID.
				pkgId--
			}
