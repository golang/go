	...
	for n, colname := range s.colName {
		...
		if strvalue, ok := s.colValue[n].(string); ok && strings.HasPrefix(strvalue, "?") {
			if strvalue == "?" {
				...
				if idx := slices.IndexFunc(args, func(a driver.NamedValue) bool { return a.Name == strvalue[1:] }); idx != -1 {
					val = args[idx].Value
				}
			}
			argPos++
		...
		for _, trow := range t.rows {
			...
			for _, wcol := range s.whereCol {
				...
				if wcol.Placeholder == "?" {
					...
					if idx := slices.IndexFunc(args, func(a driver.NamedValue) bool { return a.Name == wcol.Placeholder[1:] }); idx != -1 {
						argValue = args[idx].Value
					}
				}
