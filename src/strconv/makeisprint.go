...
func main() {
	...
	fmt.Fprint(&buf, `// Copyright 2013 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.`+"\n\n")
	fmt.Fprint(&buf, "// Code generated by go run makeisprint.go -output isprint.go; DO NOT EDIT.\n\n")
	fmt.Fprint(&buf, "package strconv\n\n")
	...
	fmt.Fprint(&buf, "var isPrint16 = []uint16{\n")
	for i := 0; i < len(range16); i += 2 {
		fmt.Fprintf(&buf, "\t%#04x, %#04x,\n", range16[i], range16[i+1])
	}
	fmt.Fprint(&buf, "}\n\n")

	fmt.Fprint(&buf, "var isNotPrint16 = []uint16{\n")
	for _, r := range except16 {
		fmt.Fprintf(&buf, "\t%#04x,\n", r)
	}
	fmt.Fprint(&buf, "}\n\n")

	fmt.Fprint(&buf, "var isPrint32 = []uint32{\n")
	for i := 0; i < len(range32); i += 2 {
		fmt.Fprintf(&buf, "\t%#06x, %#06x,\n", range32[i], range32[i+1])
	}
	fmt.Fprint(&buf, "}\n\n")

	fmt.Fprint(&buf, "var isNotPrint32 = []uint16{ // add 0x10000 to each entry\n")
	...
	fmt.Fprint(&buf, "// isGraphic lists the graphic runes not matched by IsPrint.\n")
	fmt.Fprint(&buf, "var isGraphic = []uint16{\n")
