...
var equalMACTests = []struct {
	a   HardwareAddr
	b   HardwareAddr
	out bool
}{
	{[]byte{0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}, []byte{0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}, true},
	{[]byte{0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}, []byte{0x00, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}, false},
	{[]byte{0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}, []byte{0xAA, 0xBB, 0xCC, 0xDD, 0xEE}, false},
}

func TestEqualMAC(t *testing.T) {
	for i, tt := range equalMACTests {
		out2 := tt.a.Equal(tt.b)
		if out2 != tt.out {
			t.Errorf("%d. HardwareAddr(%q).Equal(%q) = %v, want %v", i, tt.a, tt.b, out2, tt.out)
		}
	}
}
