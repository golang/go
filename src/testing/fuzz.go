...
func initFuzzFlags() {
	...
	fuzzLibFuzzerCorpus = flag.String("test.fuzzlibfuzzercorpus", "", "directory containing libFuzzer corpus files to convert and use as seed corpus")
	fuzzConvert = flag.String("test.fuzzconvert", "", "convert libFuzzer corpus to Go format for the named fuzz target (requires -test.fuzzlibfuzzercorpus)")
}
...
func (f *F) Fuzz(ff any) {
	...
	// Handle -fuzzconvert mode: convert libFuzzer corpus to Go format and exit
	if fuzzConvert != nil && *fuzzConvert == f.name {
		if fuzzLibFuzzerCorpus == nil || *fuzzLibFuzzerCorpus == "" {
			f.Fatal("-fuzzconvert requires -fuzzlibfuzzercorpus")
			return
		}
		if err := convertLibfuzzerCorpus(*fuzzLibFuzzerCorpus, f.name, types); err != nil {
			f.Fatal(err)
		}
		return
	}
	...
	//
	// Don't load the seed corpus if this is a worker process or libFuzzer mode; we won't use it.
	if f.fstate.mode != fuzzWorker && f.fstate.mode != libFuzzerMode {
		...
		// Load libFuzzer corpus if -test.fuzzlibfuzzercorpus is specified.
		// This allows using corpus files generated by libFuzzer with Go's native fuzzing.
		if fuzzLibFuzzerCorpus != nil && *fuzzLibFuzzerCorpus != "" {
			libfuzzerCorpus, err := readLibfuzzerCorpus(*fuzzLibFuzzerCorpus, types)
			if err != nil {
				f.Logf("warning: failed to read libfuzzer corpus from %s: %v", *fuzzLibFuzzerCorpus, err)
			} else {
				for i := range libfuzzerCorpus {
					libfuzzerCorpus[i].IsSeed = true
					// Note: Unlike Go corpus entries, we keep Values non-nil even for
					// coordinator mode because the Path points to raw libFuzzer binary
					// files which workers cannot re-read as Go corpus format.
				}
				f.corpus = append(f.corpus, libfuzzerCorpus...)
			}
		}
	}
	...
	case libFuzzerMode:
		// Running in libFuzzer mode.

		// On first call, run all seeds (f.Add()
		// and testdata corpus) before processing
		// libFuzzer input. This ensures seeds
		// are tested and any crashes are found
		// immediately.
		if !f.fstate.libfuzzerSeedsRun {
			f.fstate.libfuzzerSeedsRun = true

			// Get the corpus directory (set by
			// LLVMFuzzerInitialize if user provided one)
			corpusDir := GetLibFuzzerCorpusDir()
			...
			for _, seed := range f.corpus {
				...
				// Convert seed values to libFuzzer format
				seedData := serializeLibfuzzerBytes(seed.Values)

				// Write seed to corpus directory if specified
				if corpusDir != "" {
					writeLibfuzzerCorpusEntry(corpusDir, seedData)
				}

				// Run the seed
				seedSrc := newLibfuzzerSource(seedData)
				seedArgs := seedSrc.fillArgs(types)
				f.runLibfuzzerInput(fn, seedArgs)
			}

			// Load and run testdata corpus files if available
			if libfuzzerReadTestdataCorpus != nil {
				testdataCorpus, err := libfuzzerReadTestdataCorpus(f.name, types)
				if err != nil {
					// Log error but continue - testdata corpus is optional
					fmt.Fprintf(os.Stderr, "warning: failed to read testdata corpus: %v\n", err)
				}
				for _, entry := range testdataCorpus {
					if entry.Values == nil {
						continue
					}
					// Convert corpus values to libFuzzer format
					entryData := serializeLibfuzzerBytes(entry.Values)

					// Write entry to corpus directory if specified
					if corpusDir != "" {
						writeLibfuzzerCorpusEntry(corpusDir, entryData)
					}

					// Run the entry
					entrySrc := newLibfuzzerSource(entryData)
					entryArgs := entrySrc.fillArgs(types)
					f.runLibfuzzerInput(fn, entryArgs)
				}
			}
		}

		// Deserialize libFuzzer input and run
		src := newLibfuzzerSource(f.fstate.libfuzzerInput)
		args := src.fillArgs(types)

		f.runLibfuzzerInput(fn, args)
...
// runLibfuzzerInput runs the fuzz
// function with the given arguments
// in libFuzzer mode. It handles panics
// and test failures appropriately.
func (f *F) runLibfuzzerInput(
	fn reflect.Value,
	args []reflect.Value,
) {
	// Create a minimal matcher that matches everything (empty pattern)
	matchFunc := func(pat, str string) (bool, error) { return true, nil }
	matcher := newMatcher(matchFunc, "", "-test.run", "")

	// Create a minimal testState
	// for the T - many APIs require
	// tstate to be non-nil
	tstate := &testState{
		match:       matcher,
		maxParallel: 1,
	}
	...
	// Create a minimal T for this invocation
	t := &T{
		common: common{
			...
			level:   f.level + 1,
			ctx:     ctx,
			w:       os.Stderr, // Need a writer for subtests to flush to
		},
		tstate: tstate,
	}

	// Run the fuzz function in a
	// goroutine so that t.Skip and
	// t.Fatal can use runtime.Goexit()
	// properly (can't call Goexit
	// from C thread)
	done := make(chan bool)
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				panicked = r
				// Capture original stack trace before it's lost
				panicStack = make([]byte, 64*1024)
				panicStack = panicStack[:runtime.Stack(panicStack, false)]
			}
			close(done)
		}()
		fnArgs := []reflect.Value{reflect.ValueOf(t)}
		fnArgs = append(fnArgs, args...)
		fn.Call(fnArgs)
	}()
	<-done

	// Run cleanup functions registered via t.Cleanup()
	t.runCleanup(normalPanic)

	// If the fuzz function panicked, print the original stack and re-panic
	if panicked != nil {
		// Print original stack trace so libFuzzer shows the real crash location
		fmt.Fprintf(os.Stderr, "panic: %v\n\n%s\n", panicked, panicStack)
		panic(panicked)
	}

	// If the test failed (via t.Error, t.Fatal, etc.), panic to signal failure to libFuzzer
	if t.Failed() {
		panic("fuzz target reported failure")
	}
}
...
type fuzzState struct {
	deps              testDeps
	mode              fuzzMode
	libfuzzerInput    []byte // input data for libFuzzer mode
	libfuzzerSeedsRun bool   // whether f.Add() seeds have been run in libFuzzer mode
}
...
const (
	...
	fuzzCoordinator
	fuzzWorker
	libFuzzerMode // for building libFuzzer-compatible binaries
)

// libfuzzerReadTestdataCorpus is set
// by libfuzzer_corpus.go when building
// with the libfuzzer tag. It reads Go
// corpus files from the testdata directory.
var libfuzzerReadTestdataCorpus func(
	name string,
	types []reflect.Type,
) ([]corpusEntry, error)
...
// fnv64a computes the FNV-1a 64-bit hash of data.
// This is implemented inline to avoid importing hash/fnv
// which would create an import cycle (hash/fnv tests import testing).
...
// writeLibfuzzerCorpusEntry writes a seed to the corpus directory.
// The filename is an FNV-1a hash of the content for deduplication.
// If the file already exists (same hash = same content), it silently skips writing.
func writeLibfuzzerCorpusEntry(corpusDir string, data []byte) {
	filename := fmt.Sprintf("%016x", fnv64a(data))
	...
	// Write the file (ignore errors - corpus writing is best-effort)
