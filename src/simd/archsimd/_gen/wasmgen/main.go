...
var (
	...
	i8_z = []string{"swizzle", "relaxed_swizzle"}
...
func main() {
	...
	*genIntrinsicsFile = mustVal(filepath.Abs(strings.ReplaceAll(*genIntrinsicsFile, "GOROOTSRC", *gorootsrc)))
	...
	must(os.MkdirAll(filepath.Dir(*genTypesFile), 0o777))
	must(os.MkdirAll(filepath.Dir(*genOpsFile), 0o777))
	must(os.MkdirAll(filepath.Dir(*genSSAOpsFile), 0o777))
	must(os.MkdirAll(filepath.Dir(*genGenOpsFile), 0o777))
	must(os.MkdirAll(filepath.Dir(*genSSARulesFile), 0o777))
	must(os.MkdirAll(filepath.Dir(*genWasmSSAFile), 0o777))
	must(os.MkdirAll(filepath.Dir(*genIntrinsicsFile), 0o777))

	log.Println("Generating WASM SIMD files...")

	formatAndWrite(*genTypesFile, genTypes)
	formatAndWrite(*genOpsFile, genOps)
	formatAndWrite(*genSSAOpsFile, genSSAOps)
	formatAndWrite(*genWasmSSAFile, genWasmSSA)
	formatAndWrite(*genIntrinsicsFile, genIntrinsics)
	formatAndWrite(*genGenOpsFile, genGenerics)
...
func genIntrinsics(f io.Writer) {
