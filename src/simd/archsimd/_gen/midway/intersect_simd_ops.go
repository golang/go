...
type whyMissing struct {
	wasm128, amd128, amd256, amd512 bool
}
...
package simd`)
		...
		for _, elem := range elems {
			...
			for m := range allMethodNames {
				if wasm128, amd128, amd256, amd512 := methods128w[m] == nil, methods128[m] == nil, methods256[m] == nil, methods512[m] == nil; !wasm128 && !amd128 && !amd256 && !amd512 {
					intersection = append(intersection, m)
					sigForMethod[m] = methods512[m] // Use 512-bit signature (arbitrary choice, they should match)
				} else if !(wasm128 && amd128 && amd256 && amd512) {
					missing[m] = whyMissing{wasm128, amd128, amd256, amd512}
	...
	for _, aaf := range archAndFiles {
		...
		doArchWrites := func(w io.Writer) {
			...
			pf("// also allows additional useful exported declrations that would weirdly pollute archsimd.\n")
			pf("\n")
