...
func TestEnvVars(t *testing.T) {
	tmpDir := t.TempDir()
	testCert, err := os.ReadFile(filepath.Join("testdata", "test-dir.crt"))
	if err != nil {
...
func TestLoadOnDiskRootsScansDirWhenUserProvided(t *testing.T) {
	...
	t.Setenv(certFileEnv, bundlePath)
...
func TestLazyDirScanGODEBUG(t *testing.T) {
	testenv.SetGODEBUG(t, "x509lazydirscan=0")
	...
	// With x509lazydirscan=0, directories should be scanned eagerly.
	// Pool should have 2 certs (bundle + dir) and no lazy state.
	if got := roots.len(); got != 2 {
		t.Errorf("got %d certs, want 2 (GODEBUG=0 should eagerly scan dirs)", got)
	}
	if roots.lazyRoots != nil {
		t.Error("expected lazyRoots to be nil when GODEBUG disables lazy scan")
	}
}
