...
func (c *aesCipherAsm) NewCTR(iv []byte) cipher.Stream {
	...
	var ac aesctr
	ac.block = c
	ac.ctr[0] = binarylite.BigEndian.Uint64(iv[0:]) // high bits
	ac.ctr[1] = binarylite.BigEndian.Uint64(iv[8:]) // low bits
...
func (c *aesctr) refill() {
	...
	for i := 0; i < streamBufferSize; i += 16 {
		binarylite.BigEndian.PutUint64(c.buffer[i+0:], c0)
		binarylite.BigEndian.PutUint64(c.buffer[i+8:], c1)
