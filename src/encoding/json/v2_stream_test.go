...
func TestDecodeInStream(t *testing.T) {
	tests := []struct {
		...
		{CaseName: Name(""), json: `{ "` + strings.Repeat("a", 513) + `" 1 }`, expTokens: []any{
			...
			decodeThis{&SyntaxError{"invalid character '1' after object key", len64(`{ "`) + 513 + len64(`" `)}},
		}},
...
func TestTokenError(t *testing.T) {
	tests := []struct {
		...
		{in: `false`, err: io.EOF},
		{in: `  1e1000`, err: &UnmarshalTypeError{Value: "number 1e1000", Type: reflect.TypeFor[float64](), Offset: len64(`  `)}},
	}
	for _, tt := range tests {
