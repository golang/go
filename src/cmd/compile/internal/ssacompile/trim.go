...
func trim(f *ssa.Func) {
	...
	for _, b := range f.Blocks {
		...
		ns := len(s.Preds)
		p.Succs[i] = ssa.Edge{B: s, I: j}
		s.Preds[j] = ssa.Edge{B: p, I: i}

		for _, e := range b.Preds[1:] {
			p, i := e.B, e.I
			p.Succs[i] = ssa.Edge{B: s, I: len(s.Preds)}
			s.Preds = append(s.Preds, ssa.Edge{B: p, I: i})
		}
...
// instructions.
func emptyBlock(b *ssa.Block) bool {
	for _, v := range b.Values {
		if v.Op != ssaop.OpPhi {
			return false
		}
...
func trimmableBlock(b *ssa.Block) bool {
...
func mergePhi(v *ssa.Value, i int, b *ssa.Block) {
	u := v.Args[i]
	if u.Block == b {
		if u.Op != ssaop.OpPhi {
