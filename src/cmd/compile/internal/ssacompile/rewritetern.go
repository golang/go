...
func rewriteTern(f *ssa.Func) {
	if f.MaxCPUFeatures == ssa.CPUNone {
		return
	}
	...
	boolExprTrees := make(map[*ssa.Value]SIMDLogicalOP)
	...
	for v, slo := range boolExprTrees {
		...
		if slo&sloInterior == 0 && v.Block.CPUfeatures.HasFeature(ssa.CPUavx512) {
			roots = append(roots, v)
		}
	}
	slices.SortFunc(roots, func(u, v *ssa.Value) int { return int(u.ID - v.ID) }) // IDs are small enough to not care about overflow.
	...
	var rewrite func(v *ssa.Value) [3]*ssa.Value
	...
	//            y: 0 0 1 1 0 0 1 1
	//            z: 0 1 0 1 0 1 0 1
	var computeTT func(v *ssa.Value, vars [3]*ssa.Value) uint8
	...
	combine := func(a, b [3]*ssa.Value) ([3]*ssa.Value, bool) {
		var c [3]*ssa.Value
		i := 0
		for _, v := range a {
			if v == nil {
	...
	computeTT = func(v *ssa.Value, vars [3]*ssa.Value) uint8 {
		i := 0
		for ; i < len(vars); i++ {
			if vars[i] == v {
	...
	replace := func(a0 *ssa.Value, vars0 [3]*ssa.Value) {
	...
	addOne := func(vars [3]*ssa.Value, v *ssa.Value) [3]*ssa.Value {
		if vars[2] != nil {
	...
	rewrite = func(v *ssa.Value) [3]*ssa.Value {
		slo := boolExprTrees[v]
		if slo == sloInterior { // leaf node, i.e., a "variable"
			return [3]*ssa.Value{v, nil, nil}
		}
		var vars [3]*ssa.Value
		hasFeature := v.Block.CPUfeatures.HasFeature(ssa.CPUavx512)
		if slo&sloNot == sloNot {
			...
			if !(ok && v.Block.CPUfeatures.HasFeature(ssa.CPUavx512)) {
				// too many variables, or cannot rewrite current values.
				// rewrite one or both subtrees if possible
				if vars0[2] != nil && a0.Block.CPUfeatures.HasFeature(ssa.CPUavx512) {
					replace(a0, vars0)
				}
				if vars1[2] != nil && a1.Block.CPUfeatures.HasFeature(ssa.CPUavx512) {
					replace(a1, vars1)
				}
