...
func canCondSelect(v *ssa.Value, arch string, loadAddr *ssa.SparseSet) bool {
...
func floatMinMaxSelOp(cond, trueVal, falseVal *ssa.Value) ssaop.Op {
...
func canSelectPhi(v *ssa.Value, loadAddr *ssa.SparseSet, cond *ssa.Value, swap bool) bool {
...
func rewritePhiAsSelect(v *ssa.Value, swap bool, cond *ssa.Value) {
	if swap {
...
func elimIf(f *ssa.Func, loadAddr *ssa.SparseSet, dom *ssa.Block) bool {
	// See if dom is an If with one arm that
	// is trivial and succeeded by the other
	// successor of dom.
	if dom.Kind != block.BlockIf || dom.Likely != ssa.BranchUnknown {
		return false
	}
	var simple, post *ssa.Block
	...
	findBlockPos := func(b *ssa.Block) bool {
		pos := b.Pos
		for _, v := range b.Values {
	...
	// in this operation.
	setBlockPos := func(b *ssa.Block) bool {
		pos := b.Pos
		for _, v := range b.Values {
...
func isLeafPlain(b *ssa.Block) bool {
	return b.Kind == block.BlockPlain && len(b.Preds) == 1
}

func clobberBlock(b *ssa.Block) {
	b.Values = nil
	b.Preds = nil
	b.Succs = nil
	b.Aux = nil
...
// loadAddr is a set of values which are used to compute the address of a load.
// Those values are exempt from CMOV generation.
func elimIfElse(f *ssa.Func, loadAddr *ssa.SparseSet, b *ssa.Block) bool {
	// See if 'b' ends in an if/else: it should
	// have two successors, both of which are BlockPlain
	// and succeeded by the same block.
	if b.Kind != block.BlockIf || b.Likely != ssa.BranchUnknown {
		return false
	}
...
func shouldElimIfElse(no, yes, post *ssa.Block, arch string) bool {
	switch arch {
	default:
		return true
	case "amd64":
