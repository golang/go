...
func memcombineLoads(f *ssa.Func, ptrAlignments []int8) {
...
func getConst(v *ssa.Value) (int64, bool) {
...
func peelAdd(v *ssa.Value) (exp *ssa.Value, imm int64) {
	if v == nil {
		return nil, 0
	}
...
func peelShift(v *ssa.Value) (exp *ssa.Value, shift int64) {
	if v == nil {
		return nil, 0
	}
...
func splitPtr(ptr *ssa.Value) (BaseAddress, int64) {
	var idx Index
	var off int64
	for {
...
func computePtrAlignments(f *ssa.Func, ptrAlignments []int8) {
...
func ptrAlignment(ptr *ssa.Value, ptrAlignments []int8) int64 {
...
func valuePtrAlignment(v *ssa.Value, ptrAlignments []int8) int64 {
...
func combineLoads(root *ssa.Value, n int64, ptrAlignments []int8) bool {
	orOp := root.Op
...
func memcombineStores(f *ssa.Func, ptrAlignments []int8) {
	mark := f.NewSparseSet(f.NumValues())
	defer f.RetSparseSet(mark)
	var order []*ssa.Value
...
func combineStores(root *ssa.Value, ptrAlignments []int8) {
	// Helper functions.
	...
	getShiftBase := func(a []StoreRecord) *ssa.Value {
		x := a[0].store.Args[1]
		y := a[1].store.Args[1]
		switch x.Op {
	...
	isShiftBase := func(v, base *ssa.Value) bool {
		val := v.Args[1]
		switch val.Op {
		case ssaop.OpTrunc64to8, ssaop.OpTrunc64to16, ssaop.OpTrunc64to32, ssaop.OpTrunc32to8, ssaop.OpTrunc32to16, ssaop.OpTrunc16to8:
			val = val.Args[0]
	...
	shift := func(v, base *ssa.Value) int64 {
		val := v.Args[1]
		switch val.Op {
		case ssaop.OpTrunc64to8, ssaop.OpTrunc64to16, ssaop.OpTrunc64to32, ssaop.OpTrunc32to8, ssaop.OpTrunc32to16, ssaop.OpTrunc16to8:
			val = val.Args[0]
...
func truncate(b *ssa.Block, pos src.XPos, v *ssa.Value, from, to int64) *ssa.Value {
	switch from*10 + to {
	case 82:
...
func zeroExtend(b *ssa.Block, pos src.XPos, v *ssa.Value, from, to int64) *ssa.Value {
	switch from*10 + to {
	case 24:
...
func leftShift(b *ssa.Block, pos src.XPos, v *ssa.Value, shift int64) *ssa.Value {
...
func rightShift(b *ssa.Block, pos src.XPos, v *ssa.Value, shift int64) *ssa.Value {
	s := b.Func.ConstInt64(types.Types[types.TUINT64], shift)
	size := v.Type.Size()
	switch size {
	case 8:
...
func byteSwap(b *ssa.Block, pos src.XPos, v *ssa.Value) *ssa.Value {
	switch v.Type.Size() {
	case 8:
