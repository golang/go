...
func (w *walkState) walkSelect(sel *ir.SelectStmt) {
	lno := ir.SetPos(sel)
	if sel.Walked() {
	...
	init = append(init, w.walkSelectCases(sel.Cases)...)
	sel.Cases = nil

	sel.Compiled = init
	w.walkStmtList(sel.Compiled)

	base.Pos = lno
}

func (w *walkState) walkSelectCases(cases []*ir.CommClause) []ir.Node {
	ncas := len(cases)
	sellineno := base.Pos

	// optimization: zero-case select
	if ncas == 0 {
		return []ir.Node{w.mkcallstmt("block")}
	}
	...
	if ncas == 2 && dflt != nil {
		...
		case ir.OSEND:
			...
			n := n.(*ir.SendStmt)
			ch := n.Chan
			cond = w.mkcall1(chanfn("selectnbsend", 2, ch.Type()), types.Types[types.TBOOL], r.PtrInit(), ch, n.Value)
		...
		case ir.OSELRECV2:
			...
			cond = typecheck.TempAt(base.Pos, w.curfunc, types.Types[types.TBOOL])
	...
	for _, cas := range cases {
		...
		// handle this.
		if base.Flag.Race {
			r := w.mkcallstmt("selectsetpc", typecheck.NodAddr(ir.NewIndexExpr(base.Pos, pcs, ir.NewInt(base.Pos, int64(i)))))
			init = append(init, r)
		}
	}
	...
	// run the select
	base.Pos = sellineno
	chosen := typecheck.TempAt(base.Pos, w.curfunc, types.Types[types.TINT])
	recvOK := typecheck.TempAt(base.Pos, w.curfunc, types.Types[types.TBOOL])
