...
func benchmarkNilCheckDeep(b *testing.B, depth int) {
	...
	for i := 0; i < depth; i++ {
		blocs = append(blocs,
			Bloc(blockn(i),
				Valu(ptrn(i), ssaop.OpAddr, ptrType, 0, nil, "sb"),
				Valu(booln(i), ssaop.OpIsNonNil, c.config.Types.Bool, 0, nil, ptrn(i)),
...
func TestNilcheckSimple(t *testing.T) {
	...
	fun := c.Fun("entry",
		...
		Bloc("checkPtr",
			...
			Valu("bool1", ssaop.OpIsNonNil, c.config.Types.Bool, 0, nil, "ptr1"),
			If("bool1", "secondCheck", "exit")),
		Bloc("secondCheck",
			Valu("bool2", ssaop.OpIsNonNil, c.config.Types.Bool, 0, nil, "ptr1"),
			If("bool2", "extra", "exit")),
		Bloc("extra",
			Goto("exit")),
		Bloc("exit",
...
func TestNilcheckAddr(t *testing.T) {
	...
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Goto("checkPtr")),
		Bloc("checkPtr",
			Valu("ptr1", ssaop.OpAddr, ptrType, 0, nil, "sb"),
			Valu("bool1", ssaop.OpIsNonNil, c.config.Types.Bool, 0, nil, "ptr1"),
			If("bool1", "extra", "exit")),
		Bloc("extra",
			Goto("exit")),
...
func TestNilcheckAddPtr(t *testing.T) {
	...
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Goto("checkPtr")),
		Bloc("checkPtr",
			Valu("off", ssaop.OpConst64, c.config.Types.Int64, 20, nil),
			Valu("ptr1", ssaop.OpAddPtr, ptrType, 0, nil, "sb", "off"),
			Valu("bool1", ssaop.OpIsNonNil, c.config.Types.Bool, 0, nil, "ptr1"),
			If("bool1", "extra", "exit")),
		Bloc("extra",
			Goto("exit")),
...
func TestNilcheckPhi(t *testing.T) {
	...
	fun := c.Fun("entry",
		Bloc("entry",
			...
			Valu("baddr", ssaop.OpLocalAddr, c.config.Types.Bool, 0, StringToAux("b"), "sp", "mem"),
		...
		Bloc("b1",
			...
			Goto("checkPtr")),
		Bloc("b2",
			Valu("ptr2", ssaop.OpAddr, ptrType, 0, nil, "sb"),
			Goto("checkPtr")),
		// both ptr1 and ptr2 are guaranteed non-nil here
		Bloc("checkPtr",
			Valu("phi", ssaop.OpPhi, ptrType, 0, nil, "ptr1", "ptr2"),
			Valu("bool2", ssaop.OpIsNonNil, c.config.Types.Bool, 0, nil, "phi"),
			If("bool2", "extra", "exit")),
		Bloc("extra",
			Goto("exit")),
		Bloc("exit",
...
func TestNilcheckKeepRemove(t *testing.T) {
	...
	fun := c.Fun("entry",
		...
		Bloc("differentCheck",
			Valu("ptr2", ssaop.OpLoad, ptrType, 0, nil, "sb", "mem"),
			Valu("bool2", ssaop.OpIsNonNil, c.config.Types.Bool, 0, nil, "ptr2"),
			If("bool2", "secondCheck", "exit")),
		Bloc("secondCheck",
			Valu("bool3", ssaop.OpIsNonNil, c.config.Types.Bool, 0, nil, "ptr1"),
			If("bool3", "extra", "exit")),
		Bloc("extra",
			Goto("exit")),
		Bloc("exit",
...
func TestNilcheckBug(t *testing.T) {
	...
	fun := c.Fun("entry",
		...
		Bloc("extra",
			...
			Valu("store", ssaop.OpStore, types.TypeMem, 0, ptrType, "ptr1", "nilptr", "mem"),
			Goto("exit")),
		Bloc("exit",
			Valu("phi", ssaop.OpPhi, types.TypeMem, 0, nil, "mem", "store"),
			Exit("phi")))
