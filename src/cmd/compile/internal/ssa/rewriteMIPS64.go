...
func rewriteValueMIPS64_OpMIPS64MOVBUload(v *Value) bool {
	...
	for {
		...
		sym := AuxToSym(v.Aux)
		ptr := v_0
		if v_1.Op != OpMIPS64MOVBstore || AuxIntToInt32(v_1.AuxInt) != off || AuxToSym(v_1.Aux) != sym {
			break
		}
		x := v_1.Args[1]
		if ptr != v_1.Args[0] {
...
func rewriteValueMIPS64_OpMIPS64MOVHUload(v *Value) bool {
	...
	for {
		...
		sym := AuxToSym(v.Aux)
		ptr := v_0
		if v_1.Op != OpMIPS64MOVHstore || AuxIntToInt32(v_1.AuxInt) != off || AuxToSym(v_1.Aux) != sym {
			break
		}
		x := v_1.Args[1]
		if ptr != v_1.Args[0] {
