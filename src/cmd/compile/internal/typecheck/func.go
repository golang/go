...
func tcUnsafeString(n *ir.BinaryExpr) *ir.BinaryExpr {
	...
	t := n.X.Type()
	if !t.IsPtr() && t.Elem() != types.Types[types.TINT8] { //TODO(weixie): add check is *byte
		...
		// types due to incomplete C structs in cgo, and those types don't
		// have a meaningful size anyway.
		base.Errorf("unsafe.String of incomplete (or unallocatable) type not allowed")
	}

	if !checkunsafeString(&n.Y) {
		n.SetType(nil)
		return n
	}
