...
func Rewrite(w io.WriterAt, pos []int64, id string) error {
	...
	if f, cmd, ok := findMachoCodeSignature(w); ok {
		r := w.(io.ReaderAt)
		var opts codesign.Options
		var err error
		if _, opts, err = cmd.ReadSuperBlob(r); err != nil {
			return err
		}
		if codesign.Size(int64(cmd.Dataoff), opts) == int64(cmd.Datasize) {
			...
			codesign.Sign(cs, w.(io.Reader), int64(cmd.Dataoff), int64(text.Offset), int64(text.Filesz), f.Type == macho.TypeExec, opts)
