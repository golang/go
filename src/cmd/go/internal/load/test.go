...
// LibFuzzerTarget is set by the test command when -libfuzzer flag is used.
// It specifies the name of the fuzz target to build for libFuzzer.
var LibFuzzerTarget string
...
func TestPackagesAndErrors(loaderstate *modload.State, ctx context.Context, done func(), opts PackageOpts, p *Package, cover *TestCover) (pmain, ptest, pxtest *Package) {
	...
	// Build main package.
	// For libFuzzer mode, the testmain uses CGO (import "C"), so it goes in CgoFiles
	var mainGoFiles, mainCgoFiles []string
	if LibFuzzerTarget != "" {
		mainCgoFiles = []string{"_testmain.go"}
	} else {
		mainGoFiles = []string{"_testmain.go"}
	}
	pmain = &Package{
		PackagePublic: PackagePublic{
			Name:       "main",
	...
	// For libFuzzer mode, the testmain uses CGO which needs additional dependencies
	if LibFuzzerTarget != "" {
		deps = append(deps, "unsafe", "syscall", "runtime/cgo")
	}
...
func formatTestmain(t *testFuncs) ([]byte, error) {
	...
	tmpl := testmainTmpl
	// If building for libFuzzer, use the libfuzzer template
	if LibFuzzerTarget != "" {
		t.LibFuzzerTarget = LibFuzzerTarget
		// Validate that the specified fuzz target exists
		found := false
		for _, fz := range t.FuzzTargets {
			if fz.Name == LibFuzzerTarget {
				found = true
				if fz.Package == "_test" {
					t.LibFuzzerNeedTest = true
				} else if fz.Package == "_xtest" {
					t.LibFuzzerNeedXtest = true
				}
				break
			}
		}
		if !found {
			return nil, fmt.Errorf("fuzz target %q not found in package %s", LibFuzzerTarget, t.Package.ImportPath)
		}
		tmpl = libfuzzerTestmainTmpl
	}
...
// FormatLibFuzzerTestmain returns the content of the _testmain.go file for t
// when building a libFuzzer-compatible binary.
func FormatLibFuzzerTestmain(t *testFuncs) ([]byte, error) {
	var buf bytes.Buffer
	if err := libfuzzerTestmainTmpl.Execute(&buf, t); err != nil {
		return nil, err
	}
	return buf.Bytes(), nil
}
...
type testFuncs struct {
	...
	NeedXtest      bool
	Cover          *TestCover
	LibFuzzerTarget string // name of fuzz target for libfuzzer mode
	LibFuzzerNeedTest  bool // in libfuzzer mode, true if fuzz target is in _test package
	LibFuzzerNeedXtest bool // in libfuzzer mode, true if fuzz target is in _xtest package
}
...
// libfuzzerTestmainTmpl is the template for generating a libFuzzer-compatible
// test binary that exports LLVMFuzzerTestOneInput.
var libfuzzerTestmainTmpl = lazytemplate.New("libfuzzer", `
// Code generated by 'go test'. DO NOT EDIT.
...
{{if .ImportTest}}
	{{if .LibFuzzerNeedTest}}_test{{else}}_{{end}} {{.Package.ImportPath | printf "%q"}}
{{end}}
{{if .ImportXtest}}
	{{if .LibFuzzerNeedXtest}}_xtest{{else}}_{{end}} {{.Package.ImportPath | printf "%s_test" | printf "%q"}}
{{end}}
)
...
func LLVMFuzzerInitialize(argc *C.int, argv ***C.char) C.int {
	// Extract corpus directory from command line arguments.
	// libFuzzer passes corpus directories as positional arguments (non-flag args).
	// We only need the first one to write seeds to.
	n := int(*argc)
	if n > 1 {
		argvSlice := unsafe.Slice(*argv, n)
		for i := 1; i < n; i++ {
			arg := C.GoString(argvSlice[i])
			// Skip flags (arguments starting with -)
			if len(arg) > 0 && arg[0] != '-' {
				testing.SetLibFuzzerCorpusDir(arg)
				break
			}
		}
	}
...
func LLVMFuzzerTestOneInput(data *C.char, size C.size_t) C.int {
	if size > 0 {
		...
		testing.RunLibFuzzerTarget(fuzzTargets, "{{.LibFuzzerTarget}}", s)
	}
	return 0
}
